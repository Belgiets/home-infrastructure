...
```bash
cd domains/.infra/ && docker-compose -f docker-compose.local.yml up -d  
cd domains/dashboard/backend/ && npm run start:dev  
cd domains/dashboard/frontend/ && npm run dev
```