describe('config', () => {
  ...
  describe('config object', () => {
    it('should use default values when environment variables are not set', async () => {
      ...
      const reloadedConfig = configModule.config;
    ...
    it('should parse debounceTime as integer', async () => {
    ...
    it('should handle deleteAfterUpload as boolean', async () => {
      process.env.DELETE_AFTER_UPLOAD = 'false';

      const configModule = await import('./config');

      expect(configModule.config.deleteAfterUpload).toBe(false);
    });
  });

  describe('validateConfig', () => {
    it('should not throw when all required environment variables are set', async () => {
      process.env.WATCH_DIR = '/watch';
      process.env.GCS_BUCKET = 'bucket';
      process.env.MONGODB_URI = 'uri';

      const configModule = await import('./config');
      const reloadedValidate = configModule.validateConfig;

      expect(() => reloadedValidate()).not.toThrow();
    });

    it('should throw error when GCS_BUCKET is missing', async () => {
      process.env.WATCH_DIR = '/watch';
      delete process.env.GCS_BUCKET;
      process.env.MONGODB_URI = 'uri';
    ...
    it('should throw error when WATCH_DIR is missing', async () => {
      delete process.env.WATCH_DIR;
      process.env.GCS_BUCKET = 'bucket';
      process.env.MONGODB_URI = 'uri';
    ...
    it('should throw error when WATCH_DIR is empty string', async () => {
